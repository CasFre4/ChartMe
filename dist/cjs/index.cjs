...
(function (SplitStrategy) {
    SplitStrategy["PROPORTIONAL"] = "proportional";
    SplitStrategy["HALF"] = "half";
    SplitStrategy["DOMINANT"] = "dominant";
...
class ChartMe {
    ...
    preprocess() {
        ...
        const processedArr = [];
        const bundlekeys = Object.keys(this.bundle);
        // let prevsum: number|undefined 
        for (const [j, row] of this.data.entries()) {
            const rowData = Object.fromEntries(bundlekeys.map(key => [key, 0]));
            for (const x of row) {
    ...
    async saveFile(path) {
        ...
        const saveJSON = typeof window === 'undefined'
            ? (await Promise.resolve().then(function () { return require('./chunks/saveJSON-BYljFYsN.cjs'); })).saveJSON
    ...
    splitColors({ splits, fillColors, targetColors, colorBundle, splitstrategy }) {
        ...
        if (Object.values(SplitStrategy).includes(splitstrategy)) {
        ...
        for (const [_, row] of this.data.entries()) {
            const segments = biggestSegments(row);
            const longSegs = new PriorityQueue;
            for (const k of Object.keys(segments)) {
                for (let i = 0; i < newSplits + 1; i++) {
                    const dequed = segments[Number(k)].dequeue();
                    if (!(dequed === undefined)) {
                        const tmp = { [k]: dequed }; //check these lines longSegs is empty
            ...
            for (let i = 0; i < longSegs.data.length; i++) {
                const colorinfo = longSegs.data[i].item;
                const currentkey = Object.keys(colorinfo)[0];
                if (prevcolorinfo !== undefined && (currentkey === prevkey)) {
                    ...
                    const dequed = segments[Number(currentkey)].dequeue();
                    if (!(dequed === undefined)) {
                        const tmp = { [currentkey]: dequed }; //check these lines longSegs is empty
                        longSegs.enqueue(tmp, dequed[0]);
                    }
                    i = -1;
            ...
            if (!longSegs.isEmpty() && prev !== undefined) {
                while (!longSegs.isEmpty()) {
                    ...
                    if (current === undefined) {
                        ...
                        const prevIndexStart = prev[prevKey][1];
                        const currentIndexEnd = current[currentKey][0];
                        if (prevKey === currentKey) {
                            ...
                            const rowSeg = row.slice(prevIndexStart + 1, currentIndexEnd);
                            const valuesDict = {};
                            let prevAdjust = 0;
                            let currentAdjust = 0;
                            ...
                            if (prevAdjust + currentAdjust !== rowSeg.length) {
                                ...
                                const difference = (rowSeg.length - prevAdjust - currentAdjust) / 2;
                            ...
                            const newPrev = prevIndexStart + Math.floor(((currentIndexEnd - prevIndexStart) / 2));
...
function datafy(image, bundle) {
    const data = Array.from({ length: image.height }, () => Array(image.width).fill(null));
...
function distanceFlattened({ targetColor, currentColor }) {
...
function getFillColor(currentColor, colorsZipped) {
    let closestColor = [0, 0, 0, 0];
    let closestDistance = 500;
    let closestIndex = 0;
    for (const key of Object.keys(colorsZipped)) {
        const targetBundle = colorsZipped[Number(key)];
        const targetColor = targetBundle.tcolor;
...
async function recolorImage(image, bundle) {
...
function zipColors(fillColors, targetColors) {
    let zip = {};
...
function checkValues({ fillColors, targetColors, colorBundle }) {
    let fill = [];
    let target = [];
    if (fillColors === undefined && !(targetColors === undefined)) { //fill undef target def
        const gray = 255 / (targetColors.length - 1);
        ...
        const gray = 255 / (fillColors.length - 1);
    ...
    if (!(colorBundle === undefined)) { //fillTarget not undefined
        ...
        for (const key of Object.keys(colorBundle)) {
            const tmpcolor = colorBundle[Number(key)];
    ...
    if ((fill.length === 0) && (target.length === 0) && !(ftcolors === undefined)) {
        ...
        const zipped = Object.fromEntries(Object.entries(zipColors(fill, target)).filter(([_, obj]) => !tcolorSet.has(obj.tcolor)));
...
function colorCorrector(color) {
    let fin_color;
    if (typeof color === "string") {
        const tmpcolor = colorName[color];
